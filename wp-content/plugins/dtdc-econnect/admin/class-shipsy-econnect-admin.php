...
class Shipsy_Econnect_Admin {
	...
	public function multiple_track_manage( $redirect_to, $action, $post_ids ) {
		...
		if ( 'manage_multiple' === $action ) {
			...
			foreach ( $post_ids as $post_id ) {
				...
				$order_id = $order->get_order_number();
				if ( ! is_null( shipsy_get_ref_no( $order_id ) ) ) {
					if ( is_null( shipsy_get_tracking_url( $order_id ) ) ) {
						if ( shipsy_add_tracking_url( $order_id ) ) {
	...
	public function column_value( $column ) {
		...
		$shipsy_ref_no = shipsy_get_ref_no( sanitize_text_field( $order_id ) );
		if ( 'sync_order' === $column ) {
			$url = ( add_query_arg( array( 'orderid' => $order_id ), $url ) );
			if ( ! is_null( $shipsy_ref_no ) ) {
				echo esc_html( shipsy_get_ref_no( $order_id ) );
			} else {
			...
			if ( ! is_null( $shipsy_ref_no ) ) {
				$tracking_url = shipsy_get_tracking_url( sanitize_text_field( $order_id ) );
